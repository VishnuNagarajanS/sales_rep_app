...
const saveStoredDocument = (doc: DocumentItem) => {
  try {
...
const GhlIrmKycView: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      const allDeals = await getDeals(tenant?.id);
      setDeals(allDeals.filter(d => d.stage === 'qualified_investor'));
    } catch (err) {
      console.error('Failed to load KYC deals', err);
    }
  };
  ...
  const handleAdvanceStage = (deal: Deal) => {
    ...
    apiAddDealActivity(activity).catch(console.error);
...
const OriginalKYCView: React.FC = () => {
  ...
  const loadData = async () => {
    const allDeals = await getDeals(tenant?.id);
    setDeals(allDeals.filter((d: Deal) => d.stage === 'qualified_investor'));
  };
